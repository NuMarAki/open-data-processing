...
def carregar_pnad(amostra_path: str) -> pd.DataFrame:
...
def preparar_xy(df: pd.DataFrame, alvo: str, salarios_minimos_map: Optional[dict], n_sm: int) -> Tuple[pd.DataFrame, pd.Series, List[str], List[str]]:
    ...
    if alvo == "ocupado":
        ...
        renda = _renda_uso(df)
        y = (renda >= n_sm * df["salario_minimo_ano"]).astype(int)
...
def treinar_avaliar(X, y, features_num, features_cat, out_dir: Path, prefix: str, sample_weight: Optional[pd.Series] = None):
...
def main():
    ap = argparse.ArgumentParser(description="Predição usando amostra PNAD")
    ap.add_argument("--amostra", default="amostra_pnad.csv", help="Caminho para amostra_pnad.csv (sep=';')")
    ap.add_argument("--alvo", choices=["ocupado","salario_6sm","carteira","horas44","eh_ti_demografico"], default="ocupado")
    ...
    ap.add_argument("--usar_peso", action="store_true", help="Usar peso_populacional se disponível")
    args = ap.parse_args()

    df = carregar_pnad(args.amostra)
    ...
    metrics = treinar_avaliar(X, y, features_num, features_cat, out_dir, prefix, sample_weight=sw)
