# -*- coding: utf-8 -*-
"""
Predição (RAIS-only, multi-arquivos) da probabilidade de estar empregado no fechamento do ano:
- alvo: vinculo_ativo_31/12  -> normalizado para 'vinculo_ativo_3112' (1/0)

Objetivo do TCC: estimar fatores associados a "manter-se empregado" e quais campos mais importam.
Este script:
- Varre um diretório e lê múltiplos arquivos RAIS: *_processado_ti.parquet e *_processado_ti.csv
- Treina RandomForestClassifier
- Exporta importâncias por dummy e agregadas por variável original
- Exporta permutation importance
- Faz análise de estabilidade (bootstrap) dos top-20 fatores (imagem + CSV + resumo em texto)

Saídas (resultados/rais/ativo_3112):
- modelo.joblib
- rais_ativo_3112_predicoes_teste.csv
- resumo.txt (inclui seção com top-20 e estabilidade)
- importances.csv (por dummy/coluna transformada)
- importances_aggregated.csv (agregadas por variável original)
- permutation_importance.csv
- perm_importance_bootstrap_summary.csv
- perm_importance_bootstrap_top20.png
"""
...
def _collect_from_dir(rais_dir: str, patterns: List[str], rows_per_file: Optional[int], max_files: Optional[int]) -> pd.DataFrame:
    ...
    print(f"[info] Arquivos encontrados: {len(paths)}")
    ...
    print(f"[info] Total concatenado (normalizado): {len(df_all):,} linhas")
...
def main():
    ...
    print(f"[info] Varredura em: {args.rais_dir}")
    print(f"[info] Padrões: {patterns}")
    if args.rows_per_file:
        print(f"[info] Amostragem por arquivo: {args.rows_per_file} linhas")
    if args.max_files:
        print(f"[info] Limite de arquivos: {args.max_files}")
    ...
    print(f"[ok] Treino concluído. Resultados em: {out_dir.resolve()}")
    print(f"[resumo] Acc={metrics['acc']:.3f} Prec={metrics['prec']:.3f} Rec={metrics['rec']:.3f} F1={metrics['f1']:.3f} ROC={metrics['roc']:.3f} PR-AUC={metrics['ap']:.3f}")
