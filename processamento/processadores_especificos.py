...
from processamento.processador_base import ProcessadorBase
from scripts.config_manager import ConfigBase
from scripts.utils import logger, criar_caminho, listar_arquivos_por_extensao, preparar_dados_ti, ajustar_colunas_ocupacao
    ...
    def processar_arquivo(self, arquivo: str) -> Optional[pd.DataFrame]:
        try:
            ...
            if PROCESSAMENTO_TI_APLICADO_FLAG not in df.attrs:
                try:
                    from scripts.utils import preparar_dados_ti, normalizar_cbo_familia
                except ImportError:
