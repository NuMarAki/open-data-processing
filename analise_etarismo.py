...
import scipy
from utils import logger, configurar_log, criar_caminho, preparar_dados_ti, FAIXAS_ETARIAS, gerar_estatisticas_basicas
...
def ajustar_colunas_ocupacao(df):
    """
    Renomeia qualquer coluna que comece com 'cbo' para 'cbo_ocupacao'.
    """
    ...
    for col in list(df.columns):
        ...
        # Se a coluna começa com 'cbo' e não é exatamente 'cbo_ocupacao'
        if col_norm.startswith('cbo') and col != 'cbo_ocupacao':
            df.rename(columns={col: 'cbo_ocupacao'}, inplace=True)
            logger.info(f"Coluna '{col}' renomeada para 'cbo_ocupacao'")
            break
