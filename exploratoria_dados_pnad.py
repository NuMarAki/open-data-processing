...
from integracao_rais_pnad_features import augment_pnad_with_rais
from metrics_validacao_rais import validar_com_rais
from utils import classificar_faixa_etaria  # <-- novo import
...
def rf_ti_6sm(
    ...
    ano_teste=None,
    validar_grade=False,
    usar_rais_features=True,                           # <<< NOVO
    rais_dir=r"C:\TCC\dados\rais\preprocessados"       # <<< NOVO
):
    print("\n[rf_ti_6sm] Iniciando...")
...
def run_dois_alvos(
    ...
    print("\n[run_dois_alvos] Rodando os dois alvos com saídas separadas...")
    out1 = rf_ti_6sm(
...
"""
    ...
    for out in (out1, out2):
        print(f"- Alvo: {out['alvo']}")
        print(f"  Graficos:  {out['graficos_dir']}")
        print(f"  Resultados:{out['resultados_dir']}")
        print(f"  Modelos:   {out['modelos_dir']}")
        print(f"  Modelo:    {out['modelo_path']}")
        print(f"  Resumo:    {out['resumo_path']}")
        print(f"  Info:      {out['info_path']}")
    return out1, out2 """
